 ...
 *
 * Every branch here fails **at load**, naming the offending field — the same discipline
 * `parseSessionConfig` and `catalog-loader.ts`'s `parseCatalog` already apply. `catalog` is
 * wire-format JSON, identical in shape to `config/effects.json`, and is parsed/serialized
 * through `parseCatalog`/`serializeCatalog` **unmodified** — there is no second effect-catalog
 * format anywhere in this file (I1 remediation; SC-008 depends on this being true by
 * construction, not by convention).
 */

import { parseCatalog, serializeCatalog } from '../../infrastructure/effects/catalog-loader';
import type { Json } from '../../infrastructure/effects/catalog-loader';
import type { ActionRegistry } from '../runtime/action-registry';
import type {
  AssetLibrary,
  AssetLibraryEntry,
...
import { DEFAULT_CAMERA_TREATMENT, EMPTY_ASSET_LIBRARY, PROJECT_SCHEMA_VERSION } from './types';
...
function inRange(value: number, min: number, max: number, where: string): number {
  if (value < min || value > max) {
    throw new ProjectSchemaError(where + ' must lie in [' + min + ', ' + max + '], got ' + value + '.');
  }
  return value;
}
...
function parseCameraTreatment(raw: unknown, where: string): CameraTreatmentSettings {
  ...
  return {
    ...
    crop: cropRaw === null || cropRaw === undefined ? null : parseCropRect(cropRaw, where + '.crop'),
  };
}
 ...
 * {@link ProjectRepository} (contracts/project-schema.md). The exact structural inverse of
 * {@link parseProject}.
 */
